...
if (isset($_POST["deployable"])) {
	...
	for ($i = 0; $i < count($del); $i++)
	{
		...
		mysql_query("DELETE FROM `object_data` WHERE ObjectUID = '".$del[$i]."'") or die(mysql_error());
		$delresult .= $del[$i].", ";
	}
...
while ($row = mysql_fetch_array($res)) {
	if (!$serverrunning) {
		$chbox = '<td align="center" class="gear_preview" style="vertical-align: middle;"><input name="deployable[]" value="'.$row['ObjectUID'].'" type="checkbox" /></td>';
	} else {
		$chbox = '';
	}
